...
from utility import prepare_data_cdf_ppf
from collections import defaultdict

THRESHOLD = 0.5

def identifying_dominant_species(properties_json_path: str, classes_to_analyze: list[str]) -> Optional[Dict[str, list[str]]]:
    ...
    for species_class in classes_to_analyze:
        ...
        filtered_index = np.argmax(cdf_values >= 0.5)
        threholded_image_count = sorted_images[filtered_index]

        
        dominant_species = [species for species, count in zip(species_names, sorted_image_counts) if count >= threholded_image_count]
        species_data[species_class] = dominant_species
    return species_data

print(identifying_dominant_species("output/haute-garonne_properties.json", ["Aves", "Insecta"]))